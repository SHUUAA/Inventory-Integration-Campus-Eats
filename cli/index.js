    ...
    try {
      ...
      const { selectedItems } = await inquirer.prompt([
        {
          ...
          choices: sellerItems.map(item => ({ name: item.name, value: item.id }))
        }
      ]);
      ...
      const itemQuantities = await inquirer.prompt(
        selectedItems.map(itemId => ({
          type: 'number',
          name: itemId,
          message: `Quantity of ${sellerItems.find(item => item.id === itemId)?.name}:`,
      ...
      try {
        ...
        await db.runTransaction(async (transaction) => {
          ...
          for (let i = 0; i < productDocs.length; i++) {
            ...
            const newQuantity = productDoc.data().quantity - itemQuantities[selectedItems[i]];
            if (newQuantity < 0) {
              throw new Error(`Insufficient stock for product ${selectedItems[i]}.`);
            }
          }
          ...
          for (let i = 0; i < productDocs.length; i++) {
            ...
            const newQuantity = productDocs[i].data().quantity - itemQuantities[selectedItems[i]];
